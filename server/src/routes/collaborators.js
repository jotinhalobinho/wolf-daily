...
router.get("/", (req, res) => {
  if (req.user.role === "admin") {
    const rows = db.prepare("SELECT * FROM collaborators ORDER BY name ASC").all();
    return res.json(rows.map(rowToCollaborator));
  }
  if (!req.user.collaboratorId) return res.json([]);
  const row = db.prepare("SELECT * FROM collaborators WHERE id = ?").get(req.user.collaboratorId);
...
router.post("/", requireAdmin, (req, res) => {
  ...
  const existingId = db.prepare("SELECT id FROM collaborators WHERE id = ?").get(id);
  if (existingId) return res.status(400).json({ error: "Identificador já utilizado" });
  db.prepare("INSERT INTO collaborators (id, name, role, salary) VALUES (?, ?, ?, ?)").run(
    id,
    String(name).trim(),
    String(role).trim(),
    salaryNum
...
router.put("/:id", requireAdmin, (req, res) => {
  ...
  db.prepare("UPDATE collaborators SET name = ?, role = ?, salary = ? WHERE id = ?").run(name, role, salary, id);
